# Future Plans

## What's Next

### Learning Goals
- Learn Go (beyond AI assisted coding with it - i.e this portfolio lol)
- Continue my C, C++ and Java journey
- Learn Kubernetes & cloud-native architectures
- Continue to dive deep into network programming
- Contribute to major open-source projects

### Project Ideas
- **Network monitoring tool** with real-time TUI dashboard
- **Custom shell** with modern features
- **Distributed task queue** in Go

### Career
Looking for opportunities in:
- Systems and Network programming
- Developer tools
- Infrastructure & platform engineering
- Open source focused companies

---

## Get In Touch

- **GitHub:** [@adamdeleeuw](https://github.com/adamdeleeuw)
- **LinkedIn:** [@adamjdl](https://www.linkedin.com/in/adamjdl/)
- **X**: [@adamjdeleeuw](https://x.com/adamjdeleeuw)
- **Email:** [adamjdl@student.ubc.ca]

Feel free to reach out about collaborations, opportunities, or just to chat about tech (please I'm lonely)!

---

*Last updated: February 2026*
